...
export default function Navbar({ locale, description }: NavbarProps) {
  return (
    <aside className="fixed left-0 top-0 z-50 hidden h-screen w-80 border-r border-zinc-200/80 bg-[#fcfbf810] px-8 py-10 text-zinc-900 shadow-[16px_0_48px_-42px_rgba(24,24,27,0.28)] dark:border-white/10 dark:bg-[#171717] dark:text-white md:block">
      <div className="flex h-full flex-col justify-between">
          ...
          <nav className="flex flex-col gap-2">
            {navItems.map((item, index) => (
              <a
                key={item.href}
                href={item.href}
                className="group rounded-2xl border border-zinc-300 bg-white/70 px-4 py-3 transition
                 hover:border-[#AF5F3C]/25 hover:bg-[#AF5F3C]/6 dark:border-white/10 
                 dark:bg-white/[0.03] dark:hover:border-white/15 dark:hover:bg-white/[0.05]
                 "
              >
        ...
        <div className="relative z-20 flex flex-col gap-4 text-sm text-zinc-600 dark:text-white/55">
          <a
